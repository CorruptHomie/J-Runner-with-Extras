...
(function () {
  ...
  function galleryMarquee() {
    ...
    // Duration scales with content so speed stays constant regardless of item count.
    ...
    // Colour arrives per tile as it enters the viewport.
    const io = new IntersectionObserver(
      (es) => es.forEach((e) => e.isIntersecting && e.target.classList.add("shown")),
      { threshold: 0.3 }
    );
    track.querySelectorAll("figure").forEach((f) => { f.classList.add("rise"); io.observe(f); });
  }
  ...
  function trailCursor() {
    ...
    let tx = innerWidth / 2, ty = innerHeight / 2, cx = tx, cy = ty, tw = 10, cw = 10, snapped = false;
    addEventListener("pointermove", (e) => {
      const hit = e.target.closest("a, button, .device, figure.gitem, .preset, .twopt");
      if (hit) {
        const r = hit.getBoundingClientRect();
        tx = r.left + r.width / 2; ty = r.top + r.height / 2;
        tw = Math.max(r.width, r.height) * 0.62;
        snapped = true;
      } else {
        tx = e.clientX; ty = e.clientY; tw = 10; snapped = false;
      }
      dot.classList.toggle("snap", snapped);
    }, { passive: true });

    (function loop() {
      const k = snapped ? 0.22 : 0.34;
      cx += (tx - cx) * k; cy += (ty - cy) * k; cw += (tw - cw) * 0.18;
      dot.style.transform = `translate(${cx}px,${cy}px) translate(-50%,-50%)`;
      dot.style.width = dot.style.height = cw + "px";
      raf(loop);
    })();
  }
